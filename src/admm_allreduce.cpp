...
int main(int argc, char* argv[]) {
  ...
  if (iter == 0) {
    ...
    global_model.admm_params_.output_path = argv[4];

    local_model.InitModel(global_model.admm_params_.dim, arg_parser);
    if (argc > 5) {
      std::string local_load = std::string(argv[5]) + "/admm_weight_" + local_model.worker_processor_.psid_; 
      ...
      std::string global_load = std::string(argv[5]) + "/global_params_" + local_model.worker_processor_.psid_;
                               //std::to_string(local_model.worker_processor_.partid_);
  ...
  for (int r = iter; r < max_iter; ++r) {
    ...
    delete train_set;
    if (test_set->Initialize(test_name, 0, 1)) {
